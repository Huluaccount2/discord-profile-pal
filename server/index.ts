...
const main = async () => {
  ...
  if (!data?.client_id || !data?.client_secret) {
    const requestScopes = {
      ...
      client_secret: {
        ...
        instructions:
          'You can get your Discord Client Secret from the <a href="https://discord.com/developers/applications" target="_blank" style="color: lightblue;">Discord Application Dashboard</a>. You must create a new application and then under OAuth2 click "Reveal Secret" or "Reset Secret" and copy-paste that here in this field.',
      },
      redirect_url: {
  ...
  DeskThingServer.sendLog('Discord Profile Pal server started successfully.');
};
...
const handleGet = (data: SocketData) => {
  ...
  } else if (data.request === 'discord_profile') {
    DeskThingServer.sendLog('Sending Discord profile data to clients');
    discord?.sendProfileData();
  } else {
...
DeskThingServer.on('stop', () => {
  DeskThingServer.sendLog('Stopping Discord Profile Pal server');
  if (discord) {
    discord.unsubscribe();
  }
});
